...
export class UserEditProfileComponent implements OnInit, OnDestroy {
  ...
  this._loginService.getCurrentUser().subscribe((data: any) => {
    ...
    const user = data ?? this.loginDetail;
    ...
    this.contactUs.userId = user.userId;
    this.contactUs.name = user.fullName;
    this.contactUs.emailAddress = user.emailAddress;

    this.changePass.userId = user.userId; 
  });
